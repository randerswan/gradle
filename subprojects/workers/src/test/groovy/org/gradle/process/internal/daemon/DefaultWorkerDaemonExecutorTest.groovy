...
@UsesNativeServices
class DefaultWorkerDaemonExecutorTest extends Specification {
    ...
    DefaultWorkerDaemonExecutor workerExecutor

    def setup() {
        _ * fileResolver.resolveLater(_) >> factory
        _ * fileResolver.resolve(_) >> { files -> files[0] }
        _ * executorFactory.create(_ as String) >> executor
        workerExecutor = new DefaultWorkerDaemonExecutor(workerDaemonFactory, fileResolver, serverImpl.class, executorFactory, buildOperationWorkerRegistry)
    }
    ...
    def "executor executes a given runnable in a daemon"() {
        ...
        when:
        workerExecutor.submit(TestRunnable.class) { WorkerDaemonConfiguration configuration ->
        ...
        1 * workerDaemon.execute(_, _, _) >> { action, spec, operation ->
            action.execute(spec)
